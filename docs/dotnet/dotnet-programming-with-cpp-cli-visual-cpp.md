...
> 
> While C++/CLI is reliable and robust, no new feature work is planned beyond what’s necessary to ensure continued functionality. Developers should be aware that using C++/CLI pessimizes both C++ and .NET languages, as it is constrained by the language features and APIs available at the time of its design—prior to ISO C++11 and .NET Core. C++/CLI was designed based on C++98 and its [ECMA standard](https://ecma-international.org/publications-and-standards/standards/ecma-372) has not been updated to keep up with newer C++ standards since C++11. While some features in C++11 were incorporated, more recent standards, like C++20 and C++23, have no support in C++/CLI. Practically, this means that modern C++ features like concepts, coroutines and modules do not work with managed types and even for native types, the compilation result may be mixed mode binaries where the newer features are compiled to native code. For details, see this [blog article by the MSVC team](https://devblogs.microsoft.com/cppblog/cpp20-support-comes-to-cpp-cli/).
> 
> For new projects, we recommend exploring modern alternatives such as https://github.com/dotnet/ClangSharp or https://www.swig.org/, which offer more flexibility and better alignment with current language and runtime capabilities.
